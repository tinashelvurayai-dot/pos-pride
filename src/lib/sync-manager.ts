...
export async function runSync(): Promise<{ ok: number; failed: number }> {
  if (typeof navigator !== "undefined" && !navigator.onLine) return { ok: 0, failed: getQueue().length };
...
export function startSyncManager() {
  ...
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.ready
      .then((reg) => (reg as ServiceWorkerRegistration & { sync?: { register: (t: string) => Promise<void> } }).sync?.register("tillpoint-sales"))
      .catch(() => { /* noop */ });
  }
}
