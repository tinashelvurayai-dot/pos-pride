...
function CashiersPage() {
  ...
  const staff = useQuery({
    ...
    queryFn: async () => {
      ...
      return (profiles ?? []).map((p) => ({ ...p, role: byUser.get(p.id) ?? "cashier" })) as Staff[];
    },
  });

  const save = useMutation({
    mutationFn: async (payload: { id: string; full_name: string; cashier_id: string | null; active: boolean }) => {
  ...
  return (
    <div className="p-6 md:p-10">
      <header className="mb-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Manage everyone with access to your shop.</p>
      </header>

      <Card className="border-blue-100 bg-gradient-to-br from-white to-blue-50/50 p-5 shadow-sm">
        <p className="mb-4 text-sm text-muted-foreground">
          To add a cashier: share your shop's sign-up link with them. New accounts default to the cashier role. Emails are managed through the sign-in page (the cashier can reset their own email from account settings).
        </p>
          ...
          {staff.isLoading ? (
            ...
            <div key={s.id} className="flex items-center justify-between gap-3 py-4">
              <div className="min-w-0">
                ...
                <div className="truncate text-xs text-muted-foreground">Cashier ID: {s.cashier_id ?? "-"}</div>
              </div>
              <div className="flex flex-shrink-0 items-center gap-2">
                <Badge variant={s.role === "manager" ? "default" : "secondary"} className="capitalize">{s.role}</Badge>
                {s.active ? <Badge variant="outline" className="border-blue-300 text-blue-700">Active</Badge> : <Badge variant="destructive">Inactive</Badge>}
                {s.role !== "manager" && (
          ...
          {editing && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input value={editing.cashier_id ?? ""} onChange={(e) => setEditing({ ...editing, cashier_id: e.target.value })} placeholder="e.g. C-001" />
              </div>
